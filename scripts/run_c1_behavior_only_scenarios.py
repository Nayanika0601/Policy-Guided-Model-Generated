...
CONDITION = "C1"
CONDITION_NAME = "Behavior-Only Contextual Prompting"
CONCEPTS = ["variables", "data_types", "loops", "conditionals"]
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "C1_behavior_only"
...
def build_c1_prompt(
    ...
    system_prompt = (
        ...
        "based only on observable behavior. Do not reveal the answer."
    )

    lines = [
        "Situation:",
        "You are observing a learner during a beginner programming concepts quiz.",
        "",
        "Observable behavior context:",
        f"- event_type: {event.get('event_type')}",
        f"- trigger: {event.get('trigger')}",
        f"- face_detected: {obs.get('face_detected')}",
        f"- body_detected: {obs.get('body_detected')}",
        f"- gaze_on_robot: {obs.get('gaze_on_robot')}",
        f"- head_yaw_deg: {obs.get('head_yaw_deg')}",
        f"- head_pitch_deg: {obs.get('head_pitch_deg')}",
        f"- head_moving: {obs.get('head_moving')}",
        f"- no_movement_sec: {obs.get('no_movement_sec')}",
        f"- still_there_prompt: {obs.get('still_there_prompt')}",
        f"- hand_raised: {obs.get('hand_raised')}",
        f"- speech_energy: {obs.get('speech_energy')}",
    ]
...
def _is_valid_response(text: str, apply_answer_validation: bool) -> tuple[bool, str | None]:
    ...
    if _is_answer_leak(text):
        return False, "answer_leak"
    if any(
        label in text.lower()
        for label in ("scaffold", "praise", "reassure", "metacognitive")
    ):
...
def generate_c1_response(
    ...
    latency = round(time.time() - t0, 3)
    return {
        "response_text": "Let's stay focused and continue when you are ready.",
        "llm_used": False,
        "llm_latency_sec": latency,
        "llm_prompt_sent": user_prompt,
        "llm_raw_response": llm_raw_response,
...
def run_c1_quiz_event(event, learner_state, rng, run_index, event_index):
    ...
    system_prompt, user_prompt = build_c1_prompt(
        event,
        updated_learner_state,
        interaction_state,
        obs,
    )
    llm_result = generate_c1_response(
        system_prompt,
        user_prompt,
...
def run_c1_behavior_event(
    ...
    system_prompt, user_prompt = build_c1_prompt(event, learner_state, interaction_state, obs)
    llm_result = generate_c1_response(system_prompt, user_prompt, False)
    return {
        "condition": CONDITION,
        "condition_name": CONDITION_NAME,
        "timestamp": datetime.datetime.now().isoformat(timespec="seconds"),
...
def run_session(run_index: int, seed: int):
    ...
    for event in build_event_sequence(rng):
        result, learner_state = run_c1_quiz_event(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run realistic scripted C1 behavior-only scenarios."
    )
    ...
    for run_index in range(1, runs + 1):
        print(f"Running C1 scripted session {run_index}/{runs}")
    ...
    output_path = OUTPUT_DIR / f"C1_behavior_only_pipeline_{timestamp}.json"
    output_path.write_text(json.dumps(payload, indent=2))

    print("\nC1 BEHAVIOR-ONLY SUMMARY")
    print(f"Runs: {runs}")
